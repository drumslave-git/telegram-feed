...
void main() {
  ...
  testWidgets(
    'the menu of a video post leads to the autoplay and download settings',
    (tester) async {
      gw.histories[-1] = [
      ...
      await tester.pumpAndSettle();
      await tester.tap(find.text('Autoplay and download settings'));
      await settle(tester);
      await tester.pumpAndSettle();
      // Data and storage, where the automatic downloads and the Autoplay switches are.
      expect(find.byType(DataStorageScreen), findsOneWidget);
      final autoplay = find.widgetWithText(SwitchListTile, 'Videos');
