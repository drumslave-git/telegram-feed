...
- [x] S-4 Open in Telegram did nothing without the Telegram app: url_launcher throws on `tg://` instead of returning false, so the `t.me` fallback never ran. `launchFirst` tries each link and survives the exception (timeline and notification action).
