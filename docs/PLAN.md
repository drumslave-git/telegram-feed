...
**Current phase:** 0 (spikes). **Next task:** P0-1, finish login on the emulator with the spare production account (decided 2026-09-17: founder types phone and code into the spike app), then list chats and confirm `updateNewMessage`; then P0-2.
...
- [ ] P1-4 `tool/fetch_tdlib.dart` + CI job that builds `libtdjson.so` from the pinned tag and publishes it as a release asset.
...
- [ ] P3-4 `TdwebGateway` and Flutter web build; browser notifications and Web Speech TTS; hosting with COOP/COEP headers.
