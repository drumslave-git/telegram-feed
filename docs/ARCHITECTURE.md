...
- A bubble of words alone is as wide as its longest line: `IntrinsicWidth` around the bubble, which lays the text out a second time, unwrapped. A text that no string of its length could fit on one line skips that pass and takes the whole row, which is what the measurement would give anyway (`certainlyWraps`): it counts characters against the narrowest a character of the font can be, 0.19 of the font size, so it never widens a post that would have fitted. Skipping it takes about a third off the layout phase of a screen of text posts.
