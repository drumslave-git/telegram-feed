...
8. **Settings**, laid out like the official app's: my account as a profile (photo, name, username, phone, bio, Telegram ID) on top, then one row per screen — Accounts, Saved Messages; Chat settings (the text size of posts, the theme); Privacy and security (an app lock with a PIN, and the phone's fingerprint or face where I allow it, with a timeout; whether reading here marks posts read in Telegram); Notifications and sounds (the sound and vibration of rule notifications, background watching); Data and storage (storage usage and cache clearing; automatic downloads for mobile data, Wi-Fi and roaming as in the official app — a switch, Telegram's Low, Medium and High, and photos, videos and files with their limits, with larger videos' first seconds loaded ahead; and whether GIFs and videos that load by themselves autoplay); Read aloud, AI rules, Google Drive sync; About and licenses, with the version at the bottom. Log out is in the menu.
