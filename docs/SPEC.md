...
| Main screen | Tabs: "Feeds" with the list of feeds (and a button on it that makes one), one tab per Telegram chat folder listing the folder's channels, and "All channels". A feed or a channel opens as its own timeline. Channels only: groups, bots and private chats of a folder are not shown. |
...
- I can create, rename, reorder, and delete feeds. They are listed on the "Feeds" tab of the main screen; the button on that tab creates one, dragging reorders, the row's menu edits its channels, renames or deletes it.
...
2. **Home**: a magnifier that searches the posts of every channel at once, and a tab bar with Feeds (list of feeds, each with the number of its unread posts — or of its channels with unread posts, as I choose in Notifications and sounds — and a button that makes one), folder tabs counting the same way, All channels; every channel row carries the tags of the feeds it is in. Rules and Settings in the app bar. A long press on a folder tab creates a feed from the folder's channels (a one-time copy).
...
6. **Channel info**: opened from the channel's title — photo (a tap opens it on the whole screen), name, subscribers, description, link, the channels Telegram suggests as similar (a tap opens one in the official app, since this app never joins), the channel's QR code, and tabs with the shared media of the channel: Media, Files, Links, Music, Voice. No mute and no leave: notifications are the app's own rules and the app never joins or leaves a channel.
7. **Post view**: full post with media viewer, open in Telegram, share.
8. **Settings**, laid out like the official app's: my account as a profile (photo, name, username, phone, bio, Telegram ID) on top, then one row per screen — Accounts, Saved Messages; Chat settings (the text size of posts, the theme); Privacy and security (an app lock with a PIN, and the phone's fingerprint or face where I allow it, with a timeout; whether reading here marks posts read in Telegram); Notifications and sounds (the sound and vibration of rule notifications; the badge counter: whether the badges of feeds and folder tabs count unread posts or channels with unread posts; background watching); Data and storage (storage usage and cache clearing; automatic downloads for mobile data, Wi-Fi and roaming as in the official app — a switch, Telegram's Low, Medium and High, and photos, videos and files with their limits, with larger videos' first seconds loaded ahead; and whether GIFs and videos that load by themselves autoplay); Read aloud, AI rules, Google Drive sync; About and licenses, with the version at the bottom. Log out is in the menu.
