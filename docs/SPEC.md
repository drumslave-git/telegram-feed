...
8. **Settings**: an app lock (a PIN, and the phone's fingerprint or face where I allow it, with a timeout), account as a profile (photo, name, username, phone, bio, Telegram ID), logout, media autoplay, background watching, appearance (theme and the text size of posts), storage usage and cache clearing, about and licenses.
